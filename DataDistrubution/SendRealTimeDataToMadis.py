...
PutPath = '/madis/cap/etl'  # FTP put directory
BasePath = '/data'  # define source base path

FTPServer = 'awftp.fsl.noaa.gov'
User = '****'
Pass = '****'
Success = 1
...
SourcePaths = {
    ...
    '915Temp': [BasePath + '\\Radar915\\CnsTemp', '%s%02d%03d.%02d%s',
                't'],
    '915WwTemp': [BasePath + '\\Radar915\\WwTemp', '%s%02d%03d.%02d%s',
                  't'],
    ...
    '449Temp': [BasePath + '\\Radar449\\CnsTemp', '%s%02d%03d.%02d%s',
                't'],
    '449WwTemp': [BasePath + '\\Radar449\\WwTemp', '%s%02d%03d.%02d%s',
                  't'],
...
for line in SiteContents:
    ...
    for key in SourcePathKeys:
        ...
        if site == 'ctn' and key == '915WwWind':

            print 'Skipping ctn 915WWWind'
