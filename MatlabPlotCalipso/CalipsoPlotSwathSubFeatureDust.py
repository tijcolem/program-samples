...
def run(FILE_NAME, OUT_PATH):
    ...
    # Only look at  Dust.  This is bits 10-12 set to 2. 
    data_sub_feature_type[data_sub_feature_type != 2] = 0;
    
  
    for i in range(0,len(data_sub_feature_type)):
    
        if (np.any(data_sub_feature_type[i]==2)):
  ...
  #  for i in data_sub_feature_type:
   #     print i,
    ...
    long_name = 'Dust Sub Feature Type at Altitude 0 - 20.2km'
  ...
  #  fig.set_figheight(10)
   ...
   # cb.ax.set_yticklabels(['invalid', 'clear', 'cloud', 'aerosol', 'strato', 'surface', 'subsurf', 'no signal'], fontsize=5)
    cb.ax.set_yticklabels(['Other','Dust'], fontsize=5)

    # plt.show()
    pngfile = OUT_PATH + '/' + "{0}_swath_sub_feature_dust.png".format(basename)
    fig.savefig(pngfile)
    plt.clf()
